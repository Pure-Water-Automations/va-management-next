...
import { createTask, updateTaskStatus, reassignTask, claimTask, resolveClaim } from "@/lib/actions/tasks";
...
import { canManageTasks, viewForRole } from "@/lib/auth/roles";
import { canUserActOnTask } from "@/lib/services/tasks";
import { filterProjectsByClientOrg, taskClientOrgWhere } from "./scoping";
import { visibleTools, type McpActor } from "./access";
import { MCP_TOOLS } from "./protocol";

export type McpCtx = McpActor;

const BASE_URL = env.APP_BASE_URL ?? env.NEXTAUTH_URL ?? "https://dev-team.pwasecondbrain.uk";
...
export async function executeTool(name: string, args: Record<string, unknown>, ctx: McpCtx): Promise<{ text: string; isError?: boolean }> {
  ...
  const isManager = ctx.isAdmin || canManageTasks(ctx.actorRole);
  ...
  switch (name) {
    ...
    case "get_task": {
      ...
      if (!isManager && !t.claimable && !canUserActOnTask(ctx.actorId, ctx.actorRole, t)) {
